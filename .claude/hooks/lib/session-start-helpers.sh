...
: "${_PROJECT_NUDGE_IDLE_DAYS:=7}"   # suppress project nudge until idle >= N days

# Returns a one-line nudge if the workstream has no active project marker AND
# has been idle for >= _PROJECT_NUDGE_IDLE_DAYS days (per started_at). Empty string otherwise.
session_start::maybe_project_prompt() {
  ...
  if (( idle < _PROJECT_NUDGE_IDLE_DAYS )); then return 0; fi
